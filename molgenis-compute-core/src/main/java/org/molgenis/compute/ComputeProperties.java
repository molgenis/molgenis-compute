...
public class ComputeProperties
{
	...
	public void parseCommandLine(String[] args)
	{
		...
		try
		{
			...
			if (backendUrl == null) backendUrl = Parameters.DEFAULT_BACKEND_URL;

			if (errorMailAddr == null) errorMailAddr = Parameters.DEFAULT_ERROR_ADDRESS;
			...
			if (isWebWorkflow) webWorkflowLocation = changeWebLocationToRaw(webWorkflowLocation);
			...
			if (null == this.runId)
			{
				...
				this.autogenerated = ComputeProperties.AUTO_YES;
			}
	...
	private String changeWebLocationToRaw(String webWorkflowLocation)
	{
		webWorkflowLocation = webWorkflowLocation.replaceAll("github", "raw.github");
		webWorkflowLocation = webWorkflowLocation.replaceAll("/tree", "");
		return webWorkflowLocation;
	}
	...
	public Options createOptions()
	{
		...
		options.addOption(OptionBuilder
				...
				.hasArg().withLongOpt(Parameters.PARAMETERS_TO_OVERWRITE_CMDLINE)
				.create(Parameters.PARAMETERS_TO_OVERWRITE_CMDLINE_OPTION));

		options.addOption(OptionBuilder
				.withDescription("Specify address, where email will be sent in case error occurs per project").hasArg()
				.withLongOpt(Parameters.ERROR_MESSAGE_ADDR).create(Parameters.ERROR_MESSAGE_ADDR_OPTION));

		options.addOption(OptionBuilder.withDescription("Backend address, where ComputeRun will be executed").hasArg()
				.withLongOpt(Parameters.BACKEND_URL).create(Parameters.BACKEND_URL_OPTION));

		options.addOption(OptionBuilder
				.withDescription(
						"Batch specify 1) parameter name on which batch is based; 2) number of entities in batch, e.g. sample=3")
				.hasArg().create(Parameters.BATCH));
	...
	{
		if (parametersToOverwriteMap != null) return true;
		return false;
	}
