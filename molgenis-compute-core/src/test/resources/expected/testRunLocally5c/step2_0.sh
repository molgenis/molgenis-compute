...
rundir="TEST_PROPERTY(project.basedir)/target/test/benchmark/run/testRunLocally5c"
...
if [[ ! $(IFS=$'\n' sort -u <<< "${out[*]}" | wc -l | sed -e 's/^[[:space:]]*//') = 1 ]]; then echo "Error in Step 'step2': input parameter 'out' is an array with different values. Maybe 'out' is a runtime parameter with 'more variable' values than what was folded on generation-time?" >&2; exit 1; fi
