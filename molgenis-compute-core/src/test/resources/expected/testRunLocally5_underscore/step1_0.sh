...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'step1_0.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER RUNNING $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/step1_0.sh.started
...
if [[ ! $(IFS=$'\n' sort -u <<< "${input_input[*]}" | wc -l | sed -e 's/^[[:space:]]*//') = 1 ]]; then echo "Error in Step 'step1': input parameter 'input_input' is an array with different values. Maybe 'input_input' is a runtime parameter with 'more variable' values than what was folded on generation-time?" >&2; exit 1; fi
...
touch $ENVIRONMENT_DIR/step1_0.sh.finished

# Also do bookkeeping
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task step1_0.sh finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
