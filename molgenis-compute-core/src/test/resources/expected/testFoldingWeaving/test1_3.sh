...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'test1_3.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER RUNNING $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/test1_3.sh.started
...
# Save output in environment file: '$ENVIRONMENT_DIR/test1_3.env' with the output vars of this step
...
echo "test1__has__outputLALA[3]=\"${outputLALA[0]}\"" >> $ENVIRONMENT_DIR/test1_3.env

echo "" >> $ENVIRONMENT_DIR/test1_3.env
chmod 755 $ENVIRONMENT_DIR/test1_3.env
...
touch $ENVIRONMENT_DIR/test1_3.sh.finished

# Also do bookkeeping
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task test1_3.sh finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
