...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'test6_1.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER RUNNING $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/test6_1.sh.started
...
interval="2000"
path="."
source $ENVIRONMENT_DIR/test1_3.env
source $ENVIRONMENT_DIR/test1_4.env


# Connect parameters to environment
chr="2"
runtime_concat[0]=${test1__has__outputLALA[3]}
runtime_concat[1]=${test1__has__outputLALA[4]}
...
# Save output in environment file: '$ENVIRONMENT_DIR/test6_1.env' with the output vars of this step

echo "" >> $ENVIRONMENT_DIR/test6_1.env
chmod 755 $ENVIRONMENT_DIR/test6_1.env
...
touch $ENVIRONMENT_DIR/test6_1.sh.finished

# Also do bookkeeping
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task test6_1.sh finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
