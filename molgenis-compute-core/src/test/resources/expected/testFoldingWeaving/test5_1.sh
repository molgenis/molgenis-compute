...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'test5_1.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER RUNNING $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/test5_1.sh.started
...
# Save output in environment file: '$ENVIRONMENT_DIR/test5_1.env' with the output vars of this step

echo "" >> $ENVIRONMENT_DIR/test5_1.env
chmod 755 $ENVIRONMENT_DIR/test5_1.env
...
touch $ENVIRONMENT_DIR/test5_1.sh.finished

# Also do bookkeeping
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task test5_1.sh finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
