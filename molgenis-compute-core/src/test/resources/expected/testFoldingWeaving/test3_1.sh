...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'test3_1.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER RUNNING $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/test3_1.sh.started
...
if [[ ! $(IFS=$'\n' sort -u <<< "${chunk[*]}" | wc -l | sed -e 's/^[[:space:]]*//') = 1 ]]; then echo "Error in Step 'test3': input parameter 'chunk' is an array with different values. Maybe 'chunk' is a runtime parameter with 'more variable' values than what was folded on generation-time?" >&2; exit 1; fi
...
# Save output in environment file: '$ENVIRONMENT_DIR/test3_1.env' with the output vars of this step

echo "" >> $ENVIRONMENT_DIR/test3_1.env
chmod 755 $ENVIRONMENT_DIR/test3_1.env
...
touch $ENVIRONMENT_DIR/test3_1.sh.finished

# Also do bookkeeping
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task test3_1.sh finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
