...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'test1_1.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER RUNNING $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/test1_1.sh.started
...
echo "test1__has__outputLALA[1]=\"${outputLALA[0]}\"" >> $ENVIRONMENT_DIR/test1_1.env
...
touch $ENVIRONMENT_DIR/test1_1.sh.finished

# Also do bookkeeping
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task test1_1.sh finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
