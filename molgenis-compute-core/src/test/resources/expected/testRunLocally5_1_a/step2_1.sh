...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'step2_1.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER RUNNING $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/step2_1.sh.started
...
guest="Lennart"
strings[0]=${step1__has__out1[2]}
strings[1]=${step1__has__out1[3]}
strings2[0]=${step1__has__out2[2]}
strings2[1]=${step1__has__out2[3]}

# Validate that each 'value' parameter has only identical values in its list
# We do that to protect you against parameter values that might not be correctly set at runtime.
if [[ ! $(IFS=$'\n' sort -u <<< "${guest[*]}" | wc -l | sed -e 's/^[[:space:]]*//') = 1 ]]; then echo "Error in Step 'step2': input parameter 'guest' is an array with different values. Maybe 'guest' is a runtime parameter with 'more variable' values than what was folded on generation-time?" >&2; exit 1; fi
...
# Save output in environment file: '$ENVIRONMENT_DIR/step2_1.env' with the output vars of this step

echo "" >> $ENVIRONMENT_DIR/step2_1.env
chmod 755 $ENVIRONMENT_DIR/step2_1.env
...
touch $ENVIRONMENT_DIR/step2_1.sh.finished

# Also do bookkeeping
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task step2_1.sh finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
