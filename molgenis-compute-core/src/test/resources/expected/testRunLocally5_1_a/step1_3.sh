...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'step1_3.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER RUNNING $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
echo "step1__has__out1[3]=\"${out1[0]}\"" >> $ENVIRONMENT_DIR/step1_3.env
if [[ -z "$out2" ]]; then echo "In step 'step1', parameter 'out2' has no value! Please assign a value to parameter 'out2'." >&2; exit 1; fi
echo "step1__has__out2[3]=\"${out2[0]}\"" >> $ENVIRONMENT_DIR/step1_3.env
...
# Also do bookkeeping
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task step1_3.sh finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
