#!/bin/bash
#SBATCH --job-name=step0_0
#SBATCH --output=step0_0.out
#SBATCH --error=step0_0.err
...
touch step0_0.sh.finished

echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task step0_0 finished successfully" >> molgenis.bookkeeping.log
