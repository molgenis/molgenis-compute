...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'test1_0.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER RUNNING $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/test1_0.sh.started
...
runid="testParametersAllInTwoFiles"
workflow="src/main/resources/workflows/listOutOfTwo/workflow.csv"
parameters="src/main/resources/workflows/listOutOfTwo/parameters.csv,src/main/resources/workflows/listOutOfTwo/parameters1.csv"
user="mdehaan"
database="none"
backend="localhost"
port="80"
...
touch $ENVIRONMENT_DIR/test1_0.sh.finished

# Also do bookkeeping
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task test1_0.sh finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
