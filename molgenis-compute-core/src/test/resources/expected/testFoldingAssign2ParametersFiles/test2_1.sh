...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'test2_1.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER RUNNING $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/test2_1.sh.started
...
runid="testFoldingAssign2ParametersFiles"
workflow="src/main/resources/workflows/testfolding/workflow2.csv"
parameters="src/main/resources/workflows/testfolding/parameters.csv,src/main/resources/workflows/testfolding/parameters2.csv"
user="mdehaan"
database="none"
backend="localhost"
port="80"
...
# Save output in environment file: '$ENVIRONMENT_DIR/test2_1.env' with the output vars of this step

echo "" >> $ENVIRONMENT_DIR/test2_1.env
chmod 755 $ENVIRONMENT_DIR/test2_1.env
...
touch $ENVIRONMENT_DIR/test2_1.sh.finished

# Also do bookkeeping
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task test2_1.sh finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
