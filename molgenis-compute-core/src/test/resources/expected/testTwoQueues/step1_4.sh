...
function makeTmpDir {
 	base=$(basename $1)
        ...
        MC_tmpFolder=$dir/tmp_step1_4_$myMD5array/
...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'step1_4.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/step1_4.sh.started
...
# Save output in environment file: '$ENVIRONMENT_DIR/step1_4.env' with the output vars of this step

echo "" >> $ENVIRONMENT_DIR/step1_4.env
chmod 755 $ENVIRONMENT_DIR/step1_4.env
...
touch $ENVIRONMENT_DIR/step1_4.sh.finished

echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task step1_4 finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
