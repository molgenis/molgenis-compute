...
function makeTmpDir {
 	base=$(basename $1)
        ...
        MC_tmpFolder=$dir/tmp_step1_2_$myMD5array/
...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'step1_2.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task step1_2 finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
