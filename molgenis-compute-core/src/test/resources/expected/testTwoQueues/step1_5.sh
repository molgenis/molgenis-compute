...
function makeTmpDir {
 	base=$(basename $1)
        ...
        MC_tmpFolder=$dir/tmp_step1_5_$myMD5array/
...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'step1_5.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/step1_5.sh.started
...
if [[ ! $(IFS=$'\n' sort -u <<< "${queue[*]}" | wc -l | sed -e 's/^[[:space:]]*//') = 1 ]]; then echo "Error in Step 'step1': input parameter 'queue' is an array with different values. Maybe 'queue' is a runtime parameter with 'more variable' values than what was folded on generation-time?" >&2; exit 1; fi
...
# Save output in environment file: '$ENVIRONMENT_DIR/step1_5.env' with the output vars of this step

echo "" >> $ENVIRONMENT_DIR/step1_5.env
chmod 755 $ENVIRONMENT_DIR/step1_5.env
...
touch $ENVIRONMENT_DIR/step1_5.sh.finished

echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task step1_5 finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
