...
function makeTmpDir {
 	base=$(basename $1)
        ...
        MC_tmpFolder=$dir/tmp_step0_0_$myMD5array/
...
exitWithError(){
	errorCode=$1
	errorMessage=$2
	echo "$errorCode: $errorMessage --- TASK 'step0_0.sh' --- ON $(date +"%Y-%m-%d %T"), AFTER $(( ($(date +%s) - $MOLGENIS_START) / 60 )) MINUTES" >> $ENVIRONMENT_DIR/molgenis.error.log
	exit $errorCode
}
...
touch $ENVIRONMENT_DIR/step0_0.sh.started
...
rundir="TEST_PROPERTY(project.basedir)/target/test/benchmark/run/testGenerate5PBS"
runid="testGenerate5PBS"
...
touch $ENVIRONMENT_DIR/step0_0.sh.finished

echo "On $(date +"%Y-%m-%d %T"), after $(( ($(date +%s) - $MOLGENIS_START) / 60 )) minutes, task step0_0 finished successfully" >> $ENVIRONMENT_DIR/molgenis.bookkeeping.log
